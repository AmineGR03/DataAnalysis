...
\section{Cassandra - Structure et CRUD}

\subsection{Création de Structure}

\subsubsection{Création d'un Keyspace}

\begin{lstlisting}[language=SQL]
-- Créer un keyspace avec stratégie de réplication simple
...
-- Créer un keyspace avec stratégie de réplication réseau
CREATE KEYSPACE ma_base
WITH REPLICATION = {
...
\subsubsection{Création d'une Table Simple}
...
\subsubsection{Création d'une Table avec Clustering Key}

\begin{lstlisting}[language=SQL]
-- Table avec partition key et clustering key
...
\subsubsection{Création d'une Table avec Partition Key Composite}

\begin{lstlisting}[language=SQL]
-- Table avec partition key composite (entre parenthèses)
...
\subsubsection{Modification de Structure}
...
\section{MongoDB - Structure et CRUD}
...
\subsubsection{Création d'une Collection}

\begin{lstlisting}[language=JavaScript]
// MongoDB crée automatiquement les collections lors de la première insertion
// Mais on peut aussi les créer explicitement

// Créer une collection simple
db.createCollection("utilisateurs")

// Créer une collection avec options
...
\subsubsection{Création d'Index}
...
// Index composé
db.utilisateurs.createIndex({ville: 1, age: -1})
...
// Index sur tableau
db.utilisateurs.createIndex({tags: 1})
...
\section{Neo4j - Structure et CRUD}
...
\begin{lstlisting}[language=SQL]
-- Créer un index sur une propriété
...
\subsubsection{Création de Contraintes}
...
-- Contrainte de type (Neo4j 4.3+)
CREATE CONSTRAINT ON (p:Person) ASSERT p.age IS TYPED INTEGER;
...
\begin{astucebox}
\textbf{Note importante :} Dans Neo4j, les nœuds et relations sont créés dynamiquement. Il n'y a pas de schéma prédéfini comme dans les bases relationnelles. Les labels (comme \texttt{:Person}) servent à catégoriser les nœuds.
\end{astucebox}
...
\section{Redis - Structure et CRUD}
...
\begin{astucebox}
Redis ne nécessite pas de création explicite de structures. Les structures de données sont créées automatiquement lors de la première opération. Cependant, il est important de comprendre les différents types disponibles.
\end{astucebox}

\begin{itemize}
    \item \textbf{String} : Valeur simple (texte, nombre, binaire)
    \item \textbf{Hash} : Tableau associatif (clé-valeur imbriquée)
    \item \textbf{List} : Liste ordonnée de chaînes
    \item \textbf{Set} : Ensemble non ordonné de chaînes uniques
    \item \textbf{Sorted Set} : Ensemble ordonné par score
\end{itemize}

\subsubsection{Exemples de Structures}

\begin{lstlisting}
-- String : Créé automatiquement avec SET
SET utilisateur:1:nom "Dupont"

-- Hash : Créé automatiquement avec HSET
HSET utilisateur:1 nom "Dupont" age 30 ville "Paris"

-- List : Créé automatiquement avec LPUSH ou RPUSH
LPUSH commandes:1 "produit1" "produit2"

-- Set : Créé automatiquement avec SADD
SADD tags:article:1 "tech" "programming"

-- Sorted Set : Créé automatiquement avec ZADD
ZADD leaderboard 1000 "joueur1" 2000 "joueur2"
\end{lstlisting}

\subsubsection{Vérification et Gestion}
...
EXISTS utilisateur:1

-- Obtenir la taille d'une structure
STRLEN utilisateur:1:nom        -- String
HLEN utilisateur:1              -- Hash
LLEN commandes:1                 -- List
SCARD tags:article:1             -- Set
ZCARD leaderboard                -- Sorted Set

-- Supprimer une clé (tous types)
DEL utilisateur:1
\end{lstlisting}
